...
enum ShelfActions {
    ...
    static func quickLook(_ urls: [URL]) {
        ...
        // this is the one action that has to bring KlipKlick forward.
    ...
    static func compress(_ urls: [URL], named name: String, completion: @escaping (URL?) -> Void) {
        ...
        let scratch = FileManager.default.temporaryDirectory
            .appendingPathComponent("KlipKlick-zip-\(UUID().uuidString)", isDirectory: true)
    ...
    private static func destinationDirectory(for urls: [URL]) -> URL {
        ...
        )[0].appendingPathComponent("KlipKlick", isDirectory: true)
