...
struct PreferencesView: View {
    ...
    private var shelf: some View {
        VStack(spacing: 0) {
            ...
            // Stated rather than buried: everything else KlipKlik keeps between
    ...
    private var permissionsSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Screen Recording asks right here the first time. Accessibility can "
                ...
                + "so Allow opens the pane with KlipKlik already listed. Both are "
    ...
    private var storage: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            outlineButton("Uninstall KlipKlik…", action: confirmUninstall)
        }
    ...
    private func confirmUninstall() {
        let alert = NSAlert()
        alert.alertStyle = .warning
        alert.messageText = "Uninstall KlipKlik?"
        ...
        trash.messageText = "Move KlipKlik to the Trash?"
...
///
/// KlipKlik runs as an accessory so it has no Dock icon and no ⌘Tab entry. That
