...
///
/// KlipKlik lives in the menu bar with no Dock icon, so a first run is
...
struct OnboardingView: View {
    ...
    private var introText: String {
        ...
        case .permissions:
            return "macOS ties each grant to the app's code signature, so updating "
                + "KlipKlik can quietly retire it. Nothing else is wrong."
        }
    }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(mode == .welcome ? "Welcome to KlipKlik" : "KlipKlik needs permission")
