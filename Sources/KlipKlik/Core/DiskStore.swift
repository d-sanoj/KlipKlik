...
final class DiskStore {
    ...
    private let queue = DispatchQueue(label: "com.sanoj.KlipKlik.diskstore", qos: .utility)

    private init() {
        let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        root = support.appendingPathComponent("KlipKlik", isDirectory: true)
        ...
        cacheDir = caches.appendingPathComponent("KlipKlik", isDirectory: true)
